...
extension AuthImportBrainkeyInteractor: AuthImportBrainkeyInteractorInput {
    ...
    func saveSeedAndWallet(wallet: ETHWallet, seed: String) {
    ...
    private func prepareSeed(seed: String) -> (seed: String, wordsCount: Int) {
        ...
        // config valid string from array
        for index in 0..<validWordsArray.count {
            editedSeed += validWordsArray[index]
            if index != validWordsArray.count - 1 {
                editedSeed += " "
            }
        }
