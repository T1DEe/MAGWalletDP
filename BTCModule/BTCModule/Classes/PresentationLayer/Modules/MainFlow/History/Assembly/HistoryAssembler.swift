...
class HistoryModuleAssembler: Assembly {
    func assemble(container: Container) {
        container.register(HistoryInteractor.self) { (resolver, presenter: HistoryPresenter) in
            ...
            interactor.btcUpdateFacade = resolver.resolve(BTCUpdateService.self)
            interactor.btcAuthFacade = resolver.resolve(BTCAuthService.self)
            interactor.authEventDelegateHandler = resolver.resolve(AuthEventProxy.self)
            interactor.btcUpdateActionHandler = resolver.resolve(BTCUpdateEventProxy.self)
            return interactor
        }
