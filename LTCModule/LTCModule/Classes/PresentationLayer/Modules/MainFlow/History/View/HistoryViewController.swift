...
class HistoryViewController: UIViewController {
    ...
    var isLoading = false
    var responce: PagedResponse<LTCHistoryEntity>?
    ...
    private func configLocalization() {
        titleLabel.text = R.string.localization.mainFlowHistoryTitle()
    ...
    private func configCell(tableView: UITableView, indexPath: IndexPath, model: LTCHistoryEntity) -> UITableViewCell? {
    ...
    private func configSendCell(tableView: UITableView, indexPath: IndexPath, model: LTCHistoryEntity) -> UITableViewCell? {
    ...
    private func configReceiveCell(tableView: UITableView, indexPath: IndexPath, model: LTCHistoryEntity) -> UITableViewCell? {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: R.reuseIdentifier.universalHistoryCellIdentifire,
                                                       for: indexPath) else {
                                                        return nil
        }
    ...
    private func isLoadingIndex(index: IndexPath) -> Bool {
        if shouldShowLoadingCell {
            ...
            let isLoading = numberOfStorageObjects == index.row && index.row != 0
            return isLoading
        }
    ...
    private func updateHistory(response: PagedResponse<LTCHistoryEntity>) {
...
extension HistoryViewController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        let numberOfStorageObjects = responce?.data.count ?? 0
        return (shouldShowLoadingCell && numberOfStorageObjects > 0) ? numberOfStorageObjects + 1 : numberOfStorageObjects
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard tableView.cellForRow(at: indexPath) is HistoryItemCell else {
            return
        }
