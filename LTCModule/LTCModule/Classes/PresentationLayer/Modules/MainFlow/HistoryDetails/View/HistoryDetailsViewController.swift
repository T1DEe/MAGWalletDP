//
//  HistoryDetailsHistoryDetailsViewController.swift
//  LTCModule
//
...
class HistoryDetailsViewController: UIViewController {
        ...
        for index in 0..<model.details.count {
            ...
            if let amountDetail = detail as? HistoryDetailsAmountDetailScreenModel {
                ...
                view.didGetValue = { [weak self] value in
                    switch amountDetail.type {
                    case .block:
                        self?.output.actionOpenInExplorer(type: .block)
                        
                    case .fee:
                        self?.output.actionOpenInExplorer(type: .transaction)
                    }
                }
                detailView = view
            }
            ...
            if let textDetail = detail as? HistoryDetailsTextDetailScreenModel {
                ...
                view.didGetValue = { [weak self] value in
                    self?.output.actionOpenInExplorer(type: .address(value))
                }
    ...
    @IBAction func actionOpenInExplorerFromHeader(_ sender: Any) {
        output.actionOpenInExplorer(type: .transaction)
    }
}
