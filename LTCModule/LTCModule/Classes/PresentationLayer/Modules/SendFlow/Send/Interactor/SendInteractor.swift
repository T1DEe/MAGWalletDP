...
class SendInteractor {
    ...
    let currency = LTCCurrency.ltcCurrency
...
extension SendInteractor: SendInteractorInput {    
    ...
    func obtainInitialFeeEntity() -> SendFeeEntity {
        let standartFee = Amount(value: Constants.LTCConstants.LTCStandartFee, decimals: currency.decimals)
    ...
    func validateToAddress(address: String) -> SendErrorEntity? {
        guard (isValidLTCAddress(address) && !isMineLTCAddress(address)) == false else {
            return nil
        }
    ...
    private func isValidLTCAddress(_ ltcAddress: String) -> Bool {
        return transferService.isValidAddress(ltcAddress)
    }
    
    private func isMineLTCAddress(_ ltcAddress: String) -> Bool {
        guard let wallet = try? authService.getCurrentWallet() else {
            return false
        }
        return wallet.address.lowercased() == ltcAddress.lowercased()
    }
...
extension SendInteractor: LTCUpdateEventDelegate {
    func didUpdateBalance(wallet: LTCWallet) {
        updateBalanceInfo()
    }
}
