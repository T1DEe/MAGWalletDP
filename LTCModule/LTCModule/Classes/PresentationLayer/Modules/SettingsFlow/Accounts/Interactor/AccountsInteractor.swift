...
class AccountsInteractor {
    ...
    var infoService: LTCUpdateService!
...
extension AccountsInteractor: AccountsInteractorInput {
    ...
    func unsubscribeNotifications(_ wallet: LTCWallet) {
    ...
    private func updateLocalBalances(_ wallets: [LTCWallet]) {
    ...
    private func updateBalances(_ wallets: [LTCWallet]) {
    ...
    private func createModelWithoutBalances(_ wallets: [LTCWallet], currentWallet: LTCWallet) {
    ...
    private func updateModelWithNewSelectedWallet(_ wallet: LTCWallet) {
        ...
        for screenAccount in model.accounts {
            ...
            let updatedWallet = LTCWallet(address: screenAccount.wallet.address, network: currentNetwork)
    ...
    private func updateModelWithDeletedWallet(_ wallet: LTCWallet) {
        var screenAccounts = model.accounts
        screenAccounts.removeAll { $0.wallet == wallet }
        self.model = AccountsScreenModel(accounts: screenAccounts)
    }
    
    private func updateModelWithBalances(_ balances: [LTCWallet: String]) {
        var screenAccounts = [AccountsScreenAccountModel]()
        for screenAccount in model.accounts {
            guard let balance = balances[screenAccount.wallet] else {
                continue
            }
            let symbol = Constants.LTCConstants.LTCSymbol
    ...
    private func getEmptyBalances() -> NSAttributedString {
        let emptyBalance = mapAttributedString(amount: "0", symbol: Constants.LTCConstants.LTCSymbol)
        return emptyBalance
    }
    ...
    private func convertBalances(balances: [LTCWallet: String]) -> [LTCWallet: String] {
        var updatedBalances = [LTCWallet: String]()
        for (key, value) in balances {
            updatedBalances[key] = satoshiToLTC(satoshi: value)
        }
